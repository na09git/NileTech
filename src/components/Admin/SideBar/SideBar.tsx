...
const SideBar = ({ setSelectedComponent }) => {
  ...
  const Menus = [
    { title: "Dashboard", src: "Chart_fill", value: "dashboard" },
    { title: "Pending Post", src: "Chat", value: "pendingPost" },
    { title: "Add New Post", src: "User", gap: true, value: "addNewPost" },
      ...
      >
        ...
        <ul className="pt-6">
          {Menus.map((Menu, index) => (
            <li
              key={index}
              className={`hover:bg-light-white  flex cursor-pointer items-center gap-x-4 rounded-md p-2 text-base text-sm
              ${Menu.gap ? "mt-9" : "mt-2"} ${
                index === 0 && "bg-light-white"
              } `}
              onClick={() => setSelectedComponent(Menu.value)}
            >
              <img src={`/images/icon/${Menu.src}.png`} />
